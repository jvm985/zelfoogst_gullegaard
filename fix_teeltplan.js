...
async function main() {
  ...
  const getBlockByRG = (rgName) => blocks.find(b => b.rotationGroups.some(rg => rg.name === rgName));

  console.log('Nieuw meesterplan opbouwen...');
  ...
  if (b1) {
      // Bed 1 & 2: Vroege aardappel (Maart - Juni)
      ...
      // Gevolgd door Groenbemester (Juli - Dec)
      plan.push({ crop: 'Phacelia', bed: b1.beds[0], start: '2026-07-15', end: '2026-11-30' });
      
      // Bed 3, 4, 5: Late aardappel (April - September)
  ...
  if (b4) {
      ...
      // Successie sla
      plan.push({ crop: 'Sla (Kropsla)', bed: b4.beds[0], start: '2026-06-01', end: '2026-08-01' });
  ...
  if (b5) {
      ...
      plan.push({ crop: 'Knoflook', bed: b5.beds[2], start: '2026-01-01', end: '2026-07-15' }); // Overwinterd
  ...
  console.log(`Bezig met invoeren van ${plan.length} demonstratieteelten...`);
  ...
  for (const item of plan) {
    ...
    const sowDate = new Date(startDate.getTime() + (7 * 86400000)); // Zaaien 1 week na prep
    const harvestDate = new Date(sowDate.getTime() + (c.daysToMaturity * 86400000));

    await prisma.cultivation.create({
      data: {
        cropId: c.id, bedId: item.bed.id, year, quantity: 10,
        startDate, sowDate, harvestDate, endDate, status: 'PLANNED'
      }
    });
  }

  console.log('Teeltplan 2026 is nu een botanisch meesterwerk!');
}
